...
1. Find the key `clientId` and replace the existing value with the application ID (clientId) of the `sso-b2c-spa` application copied from the Azure portal.
...
1. Find the key `policies.names` and replace it with the names (IDs) of your policies/user-flows.
1. Find the key `policies.authorities` and replace it with the authority strings of your policies/user-flows.
1. Find the key `policies.authorityDomain` abd replace it with the domain of your authority.
...
#### Build and upload the `sso-b2c-spa` to an Azure Storage blob
...
1. In the `sso-b2c-spa` project source code, update your configuration file with the **Primary endpoint field** as your new **Redirect URI** (you will register this URI later).
...
### Update the Azure AD B2C app registration for `sso-b2c-spa`

1. Navigate back to to the [Azure portal](https://portal.azure.com).
1. In the left-hand navigation pane, select the **Azure AD B2C** service, and then select **App registrations**.
1. In the resulting screen, select the `msso-b2c-spa` application.
